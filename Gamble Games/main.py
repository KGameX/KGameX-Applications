...
version = '1.0.0'
birth = asctime(gmtime(1731399147))
color = 0x00ff80
creator = '@kgamex_vk'
contributors = []
...
# Dice roll
@commands.command(name = 'dice_roll', description = 'Rolls a 6-sided dice.')
async def dice_roll(interaction:discord.Interaction):
    write_on_logfile(f'{bot.user} on {asctime()}, server : {interaction.guild} ({interaction.guild_id}), user : {interaction.user}, command : dice_roll\n')
    
    await interaction.response.send_message(f'You rolled a {randint(1, 6)}.')
